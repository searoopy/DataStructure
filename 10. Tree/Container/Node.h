...
public:
	...
	//값을 전달해 자손 추가하는 함수.
	void AddChild(const Node<T>& data)
	{
...
private:
	//노드 삭제 재귀 함수.
	//재귀 함수 ->자깃자신을 호출하는 함수
	// 언제 사용하는가--? --> 자기 자신을 호출하면서 데이터의 범위가 줄어들떄.
	// 주의사항! -탈출 조건이 항상 있어야함.
	//주의사항! - 스택 오버플로우 위험이 있음.(콜스택사용하기때문)
	//로직 이해하는데 좋지만 , 루프(for등)으로 로직을 변환하는게 더 좋음.
	void RemoveChildRecursive(Node<T>* child)
	{
		...
		if (children.size() == 0)
		{
			...
			//자손 목록에서 삭제할 노드 검색(반환 타입으 iterator
